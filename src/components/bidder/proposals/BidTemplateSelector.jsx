...
import { BID_TEMPLATES } from './bidProposalTemplates'
...
export default function BidTemplateSelector() {
    const navigate = useNavigate()
    const [bidderType, setBidderType] = useState('company') // 'company' or 'freelancer'

    const handleSelectTemplate = (templateId) => {
        navigate(`/bdocuments/new?template=${templateId}&type=${bidderType}`)
    }

    const handleBlank = () => {
        navigate(`/bdocuments/new?template=blank&type=${bidderType}`)
    }
        ...
        <div className="container mx-auto p-6 max-w-5xl">
            ...
            <div className="mb-8">
                ...
                <p className="text-muted-foreground mt-2 text-base">
                    Select your bidder type and choose a template to start drafting your proposal.
                </p>
            </div>

            {/* Bidder Type Toggle */}
            <div className="mb-10 p-1 bg-muted/50 rounded-xl inline-flex">
                <button
                    onClick={() => setBidderType('freelancer')}
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${bidderType === 'freelancer'
                ...
                <button
                    onClick={() => setBidderType('company')}
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${bidderType === 'company'
                            ? 'bg-background shadow-sm text-foreground'
                            : 'text-muted-foreground hover:text-foreground'
                        }`}
                >
