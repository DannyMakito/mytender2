...
export default function SupplierContracts() {
  ...
  async function fetchSupplierContracts() {
    try {
      setLoading(true)

      // Fetch contracts where user is a signatory (supplier)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Quotation Contracts</h1>
        <p className="text-muted-foreground mt-2">Review and sign quotation contracts from your approved bids</p>
      </div>
          ...
          <Card key={contract.id} className="overflow-hidden hover:shadow-md transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground">{contract.tenders?.title || 'Quotation Tender'}</p>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <Button
                  variant="outline"
                  onClick={() => navigate(`/scontracts/${contract.id}`)}
                  className="flex-1"
                >
